...
export default function DealFlow() {
  ...
  const fetchData = useCallback(async () => {
    try {
      ...
      const dealFacts = (data.facts || []).filter(f => f.category === 'domain' || f.key?.toLowerCase().includes('deal') || f.key?.toLowerCase().includes('pitch') || f.key?.toLowerCase().includes('startup') || f.key?.toLowerCase().includes('review'));
      setFacts(dealFacts);
  ...
  return (
    <div data-testid="dealflow-page" className="space-y-4 animate-slide-in">
      {facts.length === 0 ? (
        <div className="flex flex-col items-center justify-center h-64 text-center">
          <GitPullRequest className="w-8 h-8 text-border mb-3" />
          <p className="text-sm text-muted-foreground max-w-md">No deal flow data yet. The AI will populate this as it scouts, reviews, and evaluates startups. All deal data comes from the engine's semantic memory.</p>
        </div>
      ) : (
        <div className="bg-white border border-border rounded-sm overflow-hidden">
          <div className="px-4 py-3 border-b border-border"><h3 className="text-sm font-heading font-semibold text-foreground">Deal Flow ({facts.length} entries)</h3></div>
