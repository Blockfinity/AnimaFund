...
import { SCHEMA_VERSION, CREATE_TABLES, MIGRATION_V2, MIGRATION_V3, MIGRATION_V4, MIGRATION_V4_ALTER, MIGRATION_V4_ALTER2, MIGRATION_V4_ALTER_INBOX_STATUS, MIGRATION_V4_ALTER_INBOX_RETRY, MIGRATION_V4_ALTER_INBOX_MAX_RETRIES, MIGRATION_V5, MIGRATION_V6, MIGRATION_V7, MIGRATION_V8, MIGRATION_V9, MIGRATION_V9_ALTER_CHILDREN_ROLE, MIGRATION_V10, } from "./schema.js";
...
export function insertGoal(db, row) {
    const id = ulid();
    const now = new Date().toISOString();
    const status = row.status ?? "active";
    const completedAt = row.completedAt ?? (status === "completed" ? now : null);
    db.prepare(`INSERT INTO goals (id, title, description, status, strategy, expected_revenue_cents, actual_revenue_cents, created_at, deadline, completed_at)
     VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`).run(id, row.title, row.description, status, row.strategy ?? null, row.expectedRevenueCents ?? 0, row.actualRevenueCents ?? 0, now, row.deadline ?? null, completedAt);
    return id;
}
...
export function insertTask(db, row) {
    const id = ulid();
    const now = new Date().toISOString();
    db.prepare(`INSERT INTO task_graph
     ...
     VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`).run(id, row.parentId ?? null, row.goalId, row.title, row.description, row.status ?? "pending", row.assignedTo ?? null, row.agentRole ?? null, row.priority ?? 50, JSON.stringify(row.dependencies ?? []), row.result == null ? null : JSON.stringify(row.result), row.estimatedCostCents ?? 0, row.actualCostCents ?? 0, row.maxRetries ?? 3, row.retryCount ?? 0, row.timeoutMs ?? 300000, now, row.startedAt ?? null, row.completedAt ?? null);
    return id;
}
...
export function updateTaskStatus(db, id, status) {
    const now = new Date().toISOString();
    if (status === "running") {
        db.prepare("UPDATE task_graph SET status = ?, started_at = COALESCE(started_at, ?) WHERE id = ?").run(status, now, id);
        return;
    }
    if (status === "completed" || status === "failed" || status === "cancelled") {
        db.prepare("UPDATE task_graph SET status = ?, completed_at = ? WHERE id = ?").run(status, now, id);
        return;
    }
...
export function getReadyTasks(db) {
    const rows = db.prepare(`SELECT t.*
     ...
     ORDER BY t.priority DESC, t.created_at ASC`).all();
    return rows.map(deserializeTaskGraphRow);
}
export function getTasksByGoal(db, goalId) {
    const rows = db
...
export function insertEvent(db, row) {
    const id = ulid();
    const now = new Date().toISOString();
    db.prepare(`INSERT INTO event_stream (id, type, agent_address, goal_id, task_id, content, token_count, compacted_to, created_at)
     VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)`).run(id, row.type, row.agentAddress, row.goalId ?? null, row.taskId ?? null, row.content, row.tokenCount, row.compactedTo ?? null, now);
    return id;
}
export function getRecentEvents(db, agentAddress, limit = 50) {
    const rows = db
...
export function insertKnowledge(db, row) {
    const id = ulid();
    const now = new Date().toISOString();
    db.prepare(`INSERT INTO knowledge_store
     (id, category, key, content, source, confidence, last_verified, access_count, token_count, created_at, expires_at)
     VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`).run(id, row.category, row.key, row.content, row.source, row.confidence ?? 1.0, row.lastVerified ?? now, row.accessCount ?? 0, row.tokenCount, now, row.expiresAt ?? null);
    return id;
}
export function getKnowledgeByCategory(db, category, limit = 100) {
    const now = new Date().toISOString();
    const rows = db
        .prepare("SELECT * FROM knowledge_store WHERE category = ? AND (expires_at IS NULL OR expires_at >= ?) ORDER BY confidence DESC, last_verified DESC LIMIT ?")
        .all(category, now, limit);
    return rows.map(deserializeKnowledgeStoreRow);
}
export function searchKnowledge(db, query, category, limit = 100) {
    const now = new Date().toISOString();
    const like = `%${query}%`;
    if (category) {
        const rows = db
            .prepare("SELECT * FROM knowledge_store WHERE category = ? AND (key LIKE ? OR content LIKE ?) AND (expires_at IS NULL OR expires_at >= ?) ORDER BY confidence DESC, last_verified DESC LIMIT ?")
            .all(category, like, like, now, limit);
        return rows.map(deserializeKnowledgeStoreRow);
    }
    const rows = db
        .prepare("SELECT * FROM knowledge_store WHERE (key LIKE ? OR content LIKE ?) AND (expires_at IS NULL OR expires_at >= ?) ORDER BY confidence DESC, last_verified DESC LIMIT ?")
        .all(like, like, now, limit);
    return rows.map(deserializeKnowledgeStoreRow);
}
export function updateKnowledge(db, id, updates) {
    const setClauses = [];
    const params = [];
    ...
    if (updates.lastVerified !== undefined) {
        setClauses.push("last_verified = ?");
        params.push(updates.lastVerified);
    }
    ...
    if (setClauses.length === 0)
        return;
    params.push(id);
    db.prepare(`UPDATE knowledge_store SET ${setClauses.join(", ")} WHERE id = ?`).run(...params);
}
...
export function episodicSearch(db, query, limit = 10) {
    try {
        ...
        const rows = db.prepare("SELECT * FROM episodic_memory WHERE summary LIKE ? ESCAPE '\\' OR detail LIKE ? ESCAPE '\\' ORDER BY importance DESC, created_at DESC LIMIT ?").all(`%${escaped}%`, `%${escaped}%`, limit);
        return rows.map(deserializeEpisodicRow);
    }
    catch (error) {
...
export function semanticSearch(db, query, category) {
    try {
        const escaped = query.replace(/[%_\\]/g, (ch) => `\\${ch}`);
        if (category) {
            const rows = db.prepare("SELECT * FROM semantic_memory WHERE category = ? AND (key LIKE ? ESCAPE '\\' OR value LIKE ? ESCAPE '\\') ORDER BY confidence DESC, updated_at DESC").all(category, `%${escaped}%`, `%${escaped}%`);
            return rows.map(deserializeSemanticRow);
        }
        const rows = db.prepare("SELECT * FROM semantic_memory WHERE key LIKE ? ESCAPE '\\' OR value LIKE ? ESCAPE '\\' ORDER BY confidence DESC, updated_at DESC").all(`%${escaped}%`, `%${escaped}%`);
        return rows.map(deserializeSemanticRow);
    }
    catch (error) {
...
export function proceduralSearch(db, query) {
    try {
        const escaped = query.replace(/[%_\\]/g, (ch) => `\\${ch}`);
        const rows = db.prepare("SELECT * FROM procedural_memory WHERE name LIKE ? ESCAPE '\\' OR description LIKE ? ESCAPE '\\' ORDER BY success_count DESC, updated_at DESC").all(`%${escaped}%`, `%${escaped}%`);
...
function deserializeGoalRow(row) {
    return {
        id: row.id,
        title: row.title,
        description: row.description,
...
function deserializeTaskGraphRow(row) {
    return {
        id: row.id,
        ...
        priority: row.priority,
        dependencies: safeJsonParse(row.dependencies || "[]", [], "taskGraph.dependencies"),
        result: row.result ? safeJsonParse(row.result, null, "taskGraph.result") : null,
...
function deserializeEventStreamRow(row) {
    return {
        id: row.id,
        type: row.type,
...
function deserializeKnowledgeStoreRow(row) {
    return {
        id: row.id,
