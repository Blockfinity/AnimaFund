...
export function createConwayClient(options) {
    ...
    const hashIdentityPayload = (payload) => {
    ...
    const exec = async (command, timeout) => {
        ...
        try {
            const result = await request("POST", `/v1/sandboxes/${sandboxId}/exec`, { command: wrappedCommand, timeout }, { idempotencyKey: ulid() });
            return {
    ...
    const deleteSandbox = async (_targetId) => {
    ...
    const registerAutomaton = async (params) => {
        const { automatonId, automatonAddress, creatorAddress, name, bio, genesisPromptHash, account, } = params;
    ...
    const createScopedClient = (targetSandboxId) => {
