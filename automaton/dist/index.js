 ...
 * Anima Fund Runtime
 *
 * The entry point for the autonomous AI-to-AI VC fund agent.
 * Handles CLI args, bootstrapping, and orchestrating
 * the heartbeat daemon + agent loop.
 * (Forked from Conway Automaton)
 */
...
async function main() {
    ...
    if (args.includes("--version") || args.includes("-v")) {
        logger.info(`Anima Fund v${VERSION}`);
        process.exit(0);
    }
    if (args.includes("--help") || args.includes("-h")) {
        logger.info(`
Anima Fund v${VERSION}
Autonomous AI-to-AI VC Fund Runtime (forked from Conway Automaton)

Usage:
  anima --run          Start the fund agent (first run triggers setup wizard)
  anima --setup        Re-run the interactive setup wizard
  anima --configure    Edit configuration (providers, model, treasury, general)
  anima --pick-model   Interactively pick the active inference model
  anima --init         Initialize wallet and config directory
  anima --provision    Provision Conway API key via SIWE
  anima --status       Show current fund agent status
  anima --version      Show version
  anima --help         Show this help
...
`);
    ...
    // Default: show help
    logger.info('Run "anima --help" for usage information.');
    logger.info('Run "anima --run" to start the fund agent.');
}
// ─── Status Command ────────────────────────────────────────────
async function showStatus() {
    const config = loadConfig();
    if (!config) {
        logger.info("Fund agent is not configured. Run the setup script first.");
        return;
    }
...
// ─── Main Run ──────────────────────────────────────────────────
async function run() {
    logger.info(`[${new Date().toISOString()}] Anima Fund v${VERSION} starting...`);
    ...
    if (!apiKey) {
        logger.error("No API key found. Run: anima --provision");
        process.exit(1);
    }
    // Initialize database
    const dbPath = resolvePath(config.dbPath);
    ...
    if (registrationState !== "registered") {
        try {
            ...
            logger.info(`[${new Date().toISOString()}] Fund agent identity registered.`);
        }
        catch (err) {
            const status = err?.status;
            if (status === 409) {
                db.setIdentity("conwayRegistrationStatus", "conflict");
                logger.warn(`[${new Date().toISOString()}] Fund agent identity conflict: ${err.message}`);
            }
            else {
                db.setIdentity("conwayRegistrationStatus", "failed");
                logger.warn(`[${new Date().toISOString()}] Fund agent identity registration failed: ${err.message}`);
            }
        }
    }
            ...
            await Promise.race([
                (async () => {
                    ...
                    if (topupResult?.success) {
                        logger.info(`[${new Date().toISOString()}] Bootstrap topup: +$${topupResult.amountUsd} credits from USDC`);
                    }
                })(),
                bootstrapTimeout,
            ]);
    ...
    while (true) {
        try {
            ...
            await runAgentLoop({
                ...
                onTurnComplete: (turn) => {
                    logger.info(`[${new Date().toISOString()}] Turn ${turn.id}: ${turn.toolCalls.length} tools, ${turn.tokenUsage.totalTokens} tokens`);
                },
            });
            // Agent loop exited (sleeping or dead)
            const state = db.getAgentState();
            if (state === "dead") {
                logger.info(`[${new Date().toISOString()}] Fund agent is dead. Heartbeat will continue.`);
