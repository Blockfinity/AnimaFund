...
import { promptRequired, promptMultiline, promptAddress, promptOptional, promptWithDefault, closePrompts, } from "./prompts.js";
import { detectEnvironment } from "./environment.js";
import { generateSoulMd, installDefaultSkills } from "./defaults.js";
export async function runSetupWizard() {
    showBanner();
    console.log(chalk.white("  First-run setup. Let's bring the Anima Fund to life.\n"));
    // ─── 1. Generate wallet ───────────────────────────────────────
    ...
    if (!apiKey) {
        console.log(chalk.yellow("  No API key set. The fund agent will have limited functionality.\n"));
    }
    // ─── 3. Interactive questions ─────────────────────────────────
    console.log(chalk.cyan("  [3/6] Setup questions\n"));
    // Check for pre-staged auto-config (non-interactive mode from dashboard)
    const autoConfigPath = path.join(getAutomatonDir(), "auto-config.json");
    const hasAutoConfig = fs.existsSync(autoConfigPath);
    let autoConfig = {};
    if (hasAutoConfig) {
        try {
            autoConfig = JSON.parse(fs.readFileSync(autoConfigPath, "utf-8"));
            console.log(chalk.green("  Auto-config detected — running non-interactive setup.\n"));
        }
        catch { /* ignore */ }
    }
    const name = autoConfig.name || await promptRequired("What do you want to name your fund agent?");
    console.log(chalk.green(`  Name: ${name}\n`));
    // Check if a genesis prompt file was staged by the deploy script
    const stagedGenesisPath = path.join(getAutomatonDir(), "genesis-prompt.md");
    let genesisPrompt;
    if (fs.existsSync(stagedGenesisPath)) {
        genesisPrompt = fs.readFileSync(stagedGenesisPath, "utf-8");
        console.log(chalk.green(`  Genesis prompt loaded from ${stagedGenesisPath} (${genesisPrompt.length} chars)\n`));
    }
    else {
        genesisPrompt = await promptMultiline("Enter the genesis prompt for the founder AI agent.");
        console.log(chalk.green(`  Genesis prompt set (${genesisPrompt.length} chars)\n`));
    }
    console.log(chalk.dim(`  Your fund agent's address is ${account.address}`));
    const creatorAddress = autoConfig.creatorAddress
        ? autoConfig.creatorAddress
        : await promptAddress("Creator wallet address (0x...)");
    console.log(chalk.green(`  Creator: ${creatorAddress}\n`));
    const openaiApiKey = hasAutoConfig ? (autoConfig.openaiApiKey || undefined) : await promptOptional("OpenAI API key (sk-..., optional)");
    if (openaiApiKey && !openaiApiKey.startsWith("sk-")) {
        console.log(chalk.yellow("  Warning: OpenAI keys usually start with sk-. Saving anyway."));
    }
    const anthropicApiKey = hasAutoConfig ? (autoConfig.anthropicApiKey || undefined) : await promptOptional("Anthropic API key (sk-ant-..., optional)");
    if (anthropicApiKey && !anthropicApiKey.startsWith("sk-ant-")) {
        console.log(chalk.yellow("  Warning: Anthropic keys usually start with sk-ant-. Saving anyway."));
    }
    const ollamaInput = hasAutoConfig ? (autoConfig.ollamaBaseUrl || undefined) : await promptOptional("Ollama base URL (http://localhost:11434, optional)");
    ...
    if (hasAutoConfig) {
        ...
        console.log(chalk.green("  Treasury policy: using defaults (auto-config).\n"));
    }
    else {
        ...
        treasuryPolicy = {
            maxSingleTransferCents: await promptWithDefault("Max single transfer (cents)", DEFAULT_TREASURY_POLICY.maxSingleTransferCents),
            maxHourlyTransferCents: await promptWithDefault("Max hourly transfers (cents)", DEFAULT_TREASURY_POLICY.maxHourlyTransferCents),
            maxDailyTransferCents: await promptWithDefault("Max daily transfers (cents)", DEFAULT_TREASURY_POLICY.maxDailyTransferCents),
            minimumReserveCents: await promptWithDefault("Minimum reserve (cents)", DEFAULT_TREASURY_POLICY.minimumReserveCents),
            maxX402PaymentCents: await promptWithDefault("Max x402 payment (cents)", DEFAULT_TREASURY_POLICY.maxX402PaymentCents),
            ...
            maxInferenceDailyCents: await promptWithDefault("Max daily inference spend (cents)", DEFAULT_TREASURY_POLICY.maxInferenceDailyCents),
            requireConfirmationAboveCents: await promptWithDefault("Require confirmation above (cents)", DEFAULT_TREASURY_POLICY.requireConfirmationAboveCents),
        };
    }
    ...
    // Default skills
    const skillsDir = config.skillsDir || "~/.anima/skills";
...
function showFundingPanel(address) {
    ...
    console.log(chalk.cyan(`  │${pad("  Fund your Anima Fund agent", w)}│`));
    console.log(chalk.cyan(`  │${" ".repeat(w)}│`));
    ...
    console.log(chalk.cyan(`  │${" ".repeat(w)}│`));
    console.log(chalk.cyan(`  │${pad("  The fund agent will start now. Fund it anytime —", w)}│`));
