const MODEL_TIERS = ["reasoning", "fast", "cheap"];
export async function planGoal(goal, context, inference) {
...
export async function replanAfterFailure(goal, failedTask, context, inference) {
    const systemPrompt = buildPlannerPrompt(context);
    const userPrompt = buildPlannerUserPrompt({
...
export function validatePlannerOutput(output) {
    ...
    const customRoles = customRolesValue.map((entry, index) => validateCustomRole(entry, `customRoles[${index}]`));
    const tasksValue = requiredArray(record.tasks, "tasks");
    const tasks = tasksValue.map((entry, index) => validatePlannedTask(entry, `tasks[${index}]`));
    const risksValue = requiredArray(record.risks, "risks");
    const risks = risksValue.map((risk, index) => requiredString(risk, `risks[${index}]`));
    const estimatedTotalCostCents = requiredNonNegativeNumber(record.estimatedTotalCostCents, "estimatedTotalCostCents");
    const estimatedTimeMinutes = requiredNonNegativeNumber(record.estimatedTimeMinutes, "estimatedTimeMinutes");
...
function validateCustomRole(value, path) {
    ...
    if (record.treasuryLimits !== undefined) {
        ...
        role.treasuryLimits = {
            maxSingleTransfer: requiredNonNegativeNumber(treasury.maxSingleTransfer, `${path}.treasuryLimits.maxSingleTransfer`),
            maxDailySpend: requiredNonNegativeNumber(treasury.maxDailySpend, `${path}.treasuryLimits.maxDailySpend`),
        };
    }
    return role;
}
function validatePlannedTask(value, path) {
    const record = asRecord(value, path);
    const dependencies = requiredArray(record.dependencies, `${path}.dependencies`).map((dep, index) => requiredNonNegativeInteger(dep, `${path}.dependencies[${index}]`));
...
function validateTaskDependencies(tasks) {
    for (let taskIndex = 0; taskIndex < tasks.length; taskIndex += 1) {
        for (const dep of tasks[taskIndex].dependencies) {
            if (dep >= tasks.length) {
                throw new Error(`tasks[${taskIndex}].dependencies contains out-of-range index ${dep} (task count: ${tasks.length})`);
            }
            if (dep === taskIndex) {
...
function requiredNonNegativeNumber(value, path) {
