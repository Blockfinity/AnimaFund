...
async function localWriteFile(filePath, content) {
                ...
                for (const rawToolCall of response.toolCalls) {
                    ...
                    if (!tool) {
                        ...
                        try {
                            ...
                            if (fn.name === "write_file" && typeof args.path === "string") {
    ...
    buildWorkerTools() {
        return [
            ...
            },
            {
                ...
                execute: async (args) => {
                    try {
                        const content = await this.config.conway.readFile(args.path);
                        return content.slice(0, 10_000) || "(empty file)";
                    }
                    catch {
                        try {
                            const content = await localReadFile(args.path);
                            return content.slice(0, 10_000) || "(empty file)";
                        }
                        catch (error) {
