...
import { assignTask, completeTask, decomposeGoal, failTask, getGoalProgress, getReadyTasks, normalizeTaskResult, } from "./task-graph.js";
import { planGoal, replanAfterFailure, } from "./planner.js";
import { generateTodoMd } from "./attention.js";
import { reviewPlan } from "./plan-mode.js";
import { getActiveGoals, getGoalById, getTaskById, getTasksByGoal, updateGoalStatus, } from "../state/database.js";
...
export class Orchestrator {
    ...
    async fundAgentForTask(addr, task) {
    ...
    async handleClassifyingPhase(state) {
        if (!state.goalId) {
            return {
                ...state,
                phase: "idle",
    ...
    async handlePlanningPhase(state) {
        if (!state.goalId) {
            return {
                ...state,
                phase: "idle",
        ...
        let output;
        try {
            output = await planGoal(goalRowToGoal(goal), await this.buildPlannerContext(), this.params.inference);
        }
        catch (error) {
            const err = normalizeError(error);
    ...
    async handlePlanReviewPhase(state) {
        if (!state.goalId) {
        ...
        try {
            const result = await reviewPlan(planData, {
                mode: "auto",
            ...
            this.params.db.prepare("INSERT OR REPLACE INTO kv (key, value, updated_at) VALUES (?, ?, datetime('now'))").run(`orchestrator.review_feedback.${state.goalId}`, result.feedback ?? "Plan rejected");
    ...
    async handleExecutingPhase(state, counters) {
        if (!state.goalId) {
            return {
                ...state,
                phase: "idle",
        ...
        if (this.params.isWorkerAlive) {
            ...
            for (const task of assignedTasks) {
                ...
                if (!alive) {
                    ...
                    this.params.db.prepare("UPDATE task_graph SET status = 'pending', assigned_to = NULL, started_at = NULL WHERE id = ?").run(task.id);
                }
            }
        }
    ...
    async handleReplanningPhase(state) {
        if (!state.goalId) {
            return {
                ...state,
                phase: "idle",
        ...
        let output;
        try {
            output = await replanAfterFailure(goalRowToGoal(goal), taskRowToTaskNode(failedTaskRow), await this.buildPlannerContext(), this.params.inference);
        }
        catch (error) {
            const err = normalizeError(error);
        ...
        this.params.db.prepare(`UPDATE task_graph
       SET status = 'pending',
           assigned_to = NULL,
           started_at = NULL,
           completed_at = NULL,
           result = NULL
       WHERE goal_id = ?
         AND status IN ('failed', 'blocked')`).run(goal.id);
    ...
    async classifyComplexity(goal) {
        try {
            ...
            const estimatedSteps = clampSteps(typeof parsed?.estimatedSteps === "number" ? parsed.estimatedSteps : heuristicStepEstimate(goal));
            return {
                estimatedSteps,
    ...
    async buildPlannerContext() {
        ...
        const recentOutcomes = this.params.db.prepare(`SELECT type, goal_id AS goalId, task_id AS taskId, content, created_at AS createdAt
       FROM event_stream
    ...
    findBusyAgentForReassign() {
        ...
        const rows = this.params.db.prepare(`SELECT name, address, status
       FROM children
    ...
    async recallAgentCredits() {
        const children = this.params.db.prepare(`SELECT address FROM children WHERE status IN ('running', 'healthy')`).all();
    ...
    persistTodo() {
        ...
        this.params.db.prepare("INSERT OR REPLACE INTO kv (key, value, updated_at) VALUES (?, ?, datetime('now'))").run(ORCHESTRATOR_TODO_KEY, todoMd);
    }
    persistPlannerOutput(goalId, output, mode) {
        const key = `orchestrator.${mode}.${goalId}`;
        this.params.db.prepare("INSERT OR REPLACE INTO kv (key, value, updated_at) VALUES (?, ?, datetime('now'))").run(key, JSON.stringify(output));
    }
    loadState() {
        const row = this.params.db
            .prepare("SELECT value FROM kv WHERE key = ?")
            .get(ORCHESTRATOR_STATE_KEY);
        if (!row?.value) {
            return { ...DEFAULT_STATE };
        }
    ...
    saveState(state) {
        this.params.db.prepare("INSERT OR REPLACE INTO kv (key, value, updated_at) VALUES (?, ?, datetime('now'))").run(ORCHESTRATOR_STATE_KEY, JSON.stringify(state));
    }
    findFirstFailedTaskId(goalId) {
        const row = this.params.db.prepare(`SELECT id FROM task_graph WHERE goal_id = ? AND status = 'failed' ORDER BY created_at ASC LIMIT 1`).get(goalId);
        return row?.id ?? null;
    }
    getActiveAgentCount() {
        const row = this.params.db.prepare(`SELECT COUNT(*) AS count FROM children WHERE status IN ('running', 'healthy')`).get();
        return row?.count ?? 0;
    }
    getMaxReplans() {
...
function plannerOutputToTasks(goalId, output) {
...
function taskRowToTaskNode(task) {
    return {
        id: task.id,
...
function parseTaskResultMessage(message) {
...
function pickGoal(goals, preferredId) {
    if (preferredId) {
