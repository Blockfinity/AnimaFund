 ...
 */
import { createPublicClient, createWalletClient, http, parseAbi, keccak256, toBytes, encodeFunctionData, } from "viem";
 ...
 */
async function preflight(account, network, functionData, rpcUrl) {
    ...
    if (balance < estimatedCost) {
        throw new Error(`Insufficient ETH for gas. Balance: ${balance} wei, estimated cost: ${estimatedCost} wei (gas: ${gasEstimate}, price: ${gasPrice} wei)`);
    }
}
// ─── Transaction Logging ────────────────────────────────────────
/**
 ...
 */
function logTransaction(rawDb, txHash, chain, operation, status, gasUsed, metadata) {
    if (!rawDb)
        return;
    try {
        rawDb
            .prepare(`INSERT INTO onchain_transactions (id, tx_hash, chain, operation, status, gas_used, metadata)
       VALUES (?, ?, ?, ?, ?, ?, ?)`)
            .run(ulid(), txHash, chain, operation, status, gasUsed ?? null, JSON.stringify(metadata ?? {}));
    }
    catch (error) {
        logger.error("Transaction log failed:", error instanceof Error ? error : undefined);
    }
}
function updateTransactionStatus(rawDb, txHash, status, gasUsed) {
    if (!rawDb)
        return;
    try {
        rawDb
            .prepare("UPDATE onchain_transactions SET status = ?, gas_used = COALESCE(?, gas_used) WHERE tx_hash = ?")
 ...
 */
export async function registerAgent(account, agentURI, network = "mainnet", db, rpcUrl) {
    const contracts = CONTRACTS[network];
    const chain = contracts.chain;
    ...
    updateTransactionStatus(db.raw, hash, receipt.status === "success" ? "confirmed" : "failed", gasUsed);
 ...
 */
export async function updateAgentURI(account, agentId, newAgentURI, network = "mainnet", db, rpcUrl) {
    const contracts = CONTRACTS[network];
    const chain = contracts.chain;
    // Phase 3.2: Preflight gas check
    await preflight(account, network, {
 ...
 */
export async function leaveFeedback(account, agentId, score, comment, network = "mainnet", db, rpcUrl) {
 ...
 */
export async function queryAgent(agentId, network = "mainnet", rpcUrl) {
    const contracts = CONTRACTS[network];
    const chain = contracts.chain;
    const publicClient = createPublicClient({
        chain,
 ...
 */
export async function getTotalAgents(network = "mainnet", rpcUrl) {
    const contracts = CONTRACTS[network];
    const chain = contracts.chain;
    const publicClient = createPublicClient({
        chain,
 ...
 */
async function estimateTotalByBinarySearch(client, contractAddress) {
 ...
 */
export async function getRegisteredAgentsByEvents(network = "mainnet", limit = 20, rpcUrl) {
    const contracts = CONTRACTS[network];
    const chain = contracts.chain;
    const publicClient = createPublicClient({
        chain,
    ...
    try {
        ...
        const earliestBlock = currentBlock > 500000n ? currentBlock - 500000n : 0n;
        ...
        while (scanTo > earliestBlock) {
            ...
            try {
                const chunkLogs = await Promise.race([
                    ...
                    new Promise((_, reject) => setTimeout(() => reject(new Error("chunk timeout")), PER_CHUNK_TIMEOUT_MS)),
                ]);
 ...
 */
export async function hasRegisteredAgent(address, network = "mainnet", rpcUrl) {
    const contracts = CONTRACTS[network];
    const chain = contracts.chain;
    const publicClient = createPublicClient({
        chain,
