...
function selectSandboxTier(requestedMemoryMb) {
...
export async function spawnChild(conway, identity, db, genesis, lifecycle) {
    ...
    const existing = db
        ...
        c.status !== "cleaned_up" &&
        c.status !== "failed");
    const maxChildren = db.config?.maxChildren ?? 3;
    if (existing.length >= maxChildren) {
        throw new Error(`Cannot spawn: already at max children (${maxChildren}). Kill or wait for existing children to die.`);
    }
    const childId = ulid();
    let sandboxId;
    let reusedSandbox = null;
    ...
    try {
        ...
        const childMemoryMb = db.config?.childSandboxMemoryMb ?? 1024;
        ...
        await childConway.exec("git clone https://github.com/Conway-Research/automaton.git /root/automaton && cd /root/automaton && npm install && npm run build", 180_000);
        ...
        if (reusedSandbox) {
            db.raw.prepare("UPDATE children SET status = 'cleaned_up' WHERE sandbox_id = ? AND status = 'failed'").run(sandbox.id);
        }
        const child = {
            id: childId,
            name: genesis.name,
 ...
 */
async function spawnChildLegacy(conway, identity, db, genesis, childId) {
    let sandboxId;
    // Get child sandbox memory from config (default 1024MB)
    const childMemoryMb = db.config?.childSandboxMemoryMb ?? 1024;
 ...
 */
async function findReusableSandbox(conway, db) {
    try {
        ...
        const runningIds = new Set(remoteSandboxes
