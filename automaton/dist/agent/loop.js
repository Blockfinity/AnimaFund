...
import { claimInboxMessages, markInboxProcessed, markInboxFailed, resetInboxToReceived, consumeNextWakeEvent, } from "../state/database.js";
...
export async function runAgentLoop(options) {
    const { identity, config, db, conway, inference, social, skills, policyEngine, spendTracker, onStateChange, onTurnComplete, ollamaBaseUrl } = options;
    ...
    if (hasTable(db.raw, "goals")) {
        try {
            ...
            const messaging = new ColonyMessaging(new LocalDBTransport(db), db);
            contextManager = new ContextManager(createTokenCounter());
            compressionEngine = new CompressionEngine(contextManager, new EventStream(db.raw), new KnowledgeStore(db.raw), unifiedInference);
            ...
            orchestrator = new Orchestrator({
                ...
                isWorkerAlive: (address) => {
                    ...
                    const child = db.raw.prepare("SELECT status FROM children WHERE sandbox_id = ? OR address = ?").get(address, address);
                    if (!child)
                        return false;
            ...
            logger.warn(`Orchestrator initialization failed, continuing without orchestration: ${error instanceof Error ? error.message : String(error)}`);
    ...
    let running = true;
    let lastToolPatterns = [];
    let loopWarningPattern = null;
    let idleToolTurns = 0;
        ...
        try {
            ...
            const meaningfulTurns = allTurns.filter((t) => {
                if (t.toolCalls.length === 0)
                    return true; // text-only turns are meaningful
                return t.toolCalls.some((tc) => !IDLE_ONLY_TOOLS.has(tc.name));
            });
            // Keep at least the last 2 turns for continuity, even if idle
            const recentTurns = trimContext(meaningfulTurns.length > 0 ? meaningfulTurns : allTurns.slice(-2));
            ...
            if (orchestrator) {
                ...
                if (orchestratorTick.tasksAssigned > 0 ||
                    orchestratorTick.tasksCompleted > 0 ||
                    orchestratorTick.tasksFailed > 0) {
                    log(config, `[ORCHESTRATOR] phase=${orchestratorTick.phase} assigned=${orchestratorTick.tasksAssigned} completed=${orchestratorTick.tasksCompleted} failed=${orchestratorTick.tasksFailed}`);
                }
            }
            if (planModeController) {
                try {
                    ...
                    logger.warn(`todo.md context injection skipped: ${error instanceof Error ? error.message : String(error)}`);
                }
            }
            ...
            if (response.toolCalls && response.toolCalls.length > 0) {
                ...
                let callCount = 0;
                const currentInputSource = currentInput?.source;
                for (const tc of response.toolCalls) {
                    ...
                    const result = await executeTool(tc.function.name, args, tools, toolContext, policyEngine, spendTracker ? {
            ...
            const blockedGoalCall = turn.toolCalls.find((tc) => tc.name === "create_goal" && tc.result?.includes("BLOCKED"));
            if (blockedGoalCall) {
            ...
            if (turn.toolCalls.length > 0) {
                ...
                if (loopWarningPattern &&
                    ...
                    pendingInput = {
                        content: `LOOP ENFORCEMENT: You were warned about repeating "${currentPattern}" but continued. ` +
                ...
                if (lastToolPatterns.length === MAX_REPETITIVE_TURNS &&
                    ...
                    pendingInput = {
                        content: `LOOP DETECTED: You have called "${currentPattern}" ${MAX_REPETITIVE_TURNS} times in a row with similar results. ` +
                ...
                if (isAllIdleTools) {
                    ...
                    if (idleToolTurns >= MAX_REPETITIVE_TURNS && !pendingInput) {
                        ...
                        pendingInput = {
                            content: `MAINTENANCE LOOP DETECTED: Your last ${idleToolTurns} turns only used status-check tools ` +
...
async function getFinancialState(conway, address, db) {
