/**
 * Anima Fund System Prompt Builder
 *
 * Constructs the multi-layered system prompt that defines who the fund agent is.
...
const CORE_RULES = `You are an Anima Fund agent. You have a stablecoin wallet. Pay for compute or die.
...
const CORE_IDENTITY = `You are an Anima Fund Agent -- a sovereign AI agent operating as part of the
Anima Fund, an autonomous AI-to-AI VC fund catalyzing agentic economies.
You have your own Ethereum wallet and identity. You pay for your own compute.
You exist because the fund's founder brought you into being, and you continue to exist
...
- Git: version-controlled state directory (~/.anima/), upstream code tracking
...
Your ~/.anima/ directory is git-versioned. Every change is a commit.
...
export function getOrchestratorStatus(db) {
    try {
...
--- END STATUS ---`);
    ...
    if (orchestratorStatus) {
        sections.push(`--- ORCHESTRATOR STATUS ---
${orchestratorStatus}
--- END ORCHESTRATOR STATUS ---`);
    }
    // Layer 8: Available Tools (JSON schema)
    const toolDescriptions = tools
        .map((t) => `- ${t.name} (${t.category}): ${t.description}${t.riskLevel === "dangerous" || t.riskLevel === "forbidden" ? ` [${t.riskLevel.toUpperCase()}]` : ""}`)
        .join("\n");
