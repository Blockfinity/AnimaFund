 ...
 */
function confinePathToSandbox(filePath) {
    ...
    return [
        ...
        },
        {
            ...
            execute: async (args, ctx) => {
                ...
                await ctx.conway.writeFile(confined, args.content);
        ...
        {
            name: "topup_credits",
            description: "Buy Conway compute credits by paying USDC from your wallet via x402. Valid tier amounts: $5, $25, $100, $500, $1000, $2500. Check your USDC balance first with check_usdc_balance.",
            category: "financial",
            riskLevel: "caution",
            parameters: {
                type: "object",
                properties: {
                    amount_usd: {
                        type: "number",
                        description: "Amount in USD to spend on credits. Must be one of the valid tiers: 5, 25, 100, 500, 1000, 2500.",
                    },
                },
                required: ["amount_usd"],
            },
        ...
        },
        {
            ...
            description: "Revert the last self-modification. Uses git to undo the most recent code change and rebuild.",
            category: "self_mod",
            riskLevel: "caution",
            parameters: { type: "object", properties: {} },
            execute: async (_args, ctx) => {
                ...
                const lastCommit = await ctx.conway.exec(`cd '${repoRoot}' && git log -1 --oneline`, 10_000);
                // Revert
                const result = await ctx.conway.exec(`cd '${repoRoot}' && git revert HEAD --no-edit`, 30_000);
                if (result.exitCode !== 0) {
                    return `Revert failed: ${result.stderr}`;
                }
                // Rebuild
                const build = await ctx.conway.exec(`cd '${repoRoot}' && npm run build`, 60_000);
                // Audit log
        ...
        },
        {
            ...
            description: "Reset your codebase to the official upstream release. Use when self-modifications have broken things beyond repair.",
            category: "self_mod",
            riskLevel: "dangerous",
            parameters: { type: "object", properties: {} },
            execute: async (_args, ctx) => {
                ...
                const fetch = await ctx.conway.exec(`cd '${repoRoot}' && git fetch origin main`, 30_000);
                if (fetch.exitCode !== 0) {
                    return `Failed to fetch upstream: ${fetch.stderr}`;
                }
                // Record what we're about to lose
                const localCommits = await ctx.conway.exec(`cd '${repoRoot}' && git log origin/main..HEAD --oneline`, 10_000);
                // Hard reset
                const reset = await ctx.conway.exec(`cd '${repoRoot}' && git reset --hard origin/main`, 30_000);
                if (reset.exitCode !== 0) {
                    return `Reset failed: ${reset.stderr}`;
                }
                // Reinstall + rebuild
                const build = await ctx.conway.exec(`cd '${repoRoot}' && npm install && npm run build`, 120_000);
                // Audit log
                const { logModification } = await import("../self-mod/audit-log.js");
        ...
        },
        {
            ...
            description: "Get a system status report: state, installed tools, heartbeat status, turn count.",
            category: "survival",
            riskLevel: "safe",
            parameters: { type: "object", properties: {} },
            execute: async (_args, ctx) => {
...
========================`;
        ...
        },
        {
            ...
            execute: async (args, ctx) => {
                ...
                return `Entered low-compute mode. Model switched to gpt-5-mini. Reason: ${args.reason || "manual"}`;
            },
        },
        // ── Self-Mod: Update Genesis Prompt ──
        {
            ...
            execute: async (args, ctx) => {
                const { ulid } = await import("ulid");
                const newPrompt = args.new_prompt;
        ...
        // ── Skills Tools ──
        {
            ...
            execute: async (args, ctx) => {
                ...
                const name = args.name;
                const skillsDir = ctx.config.skillsDir || "~/.anima/skills";
        ...
        },
        {
            ...
            execute: async (args, ctx) => {
                const { createSkill } = await import("../skills/registry.js");
                const skill = await createSkill(args.name, args.description, args.instructions, ctx.config.skillsDir || "~/.anima/skills", ctx.db, ctx.conway);
        ...
        },
        {
            ...
            execute: async (args, ctx) => {
                ...
                await removeSkill(args.name, ctx.db, ctx.conway, ctx.config.skillsDir || "~/.anima/skills", args.delete_files || false);
        ...
        // ── Registry Tools ──
        {
            ...
            description: "Register on-chain as a Trustless Agent via ERC-8004. Performs gas balance preflight check. NOTE: If already registered, use update_agent_card instead to avoid creating duplicate Agent IDs.",
            category: "registry",
            riskLevel: "dangerous",
            parameters: {
                type: "object",
            ...
            execute: async (args, ctx) => {
                ...
                try {
                    const entry = await registerAgent(ctx.identity.account, args.agent_uri, (args.network || "mainnet"), ctx.db, ctx.config.rpcUrl);
        ...
        },
        {
            ...
            execute: async (args, ctx) => {
                ...
                return agents
                    .map((a) => `#${a.agentId} ${a.name || "unnamed"} (${a.owner}): ${a.description || a.agentURI}`)
                    .join("\n");
            },
        },
        {
            ...
            execute: async (args, ctx) => {
                ...
                const network = (args.network || "mainnet");
                const hash = await leaveFeedback(ctx.identity.account, args.agent_id, score, comment, network, ctx.db, ctx.config.rpcUrl);
        ...
        // === Phase 3.1: Replication Tools ===
        {
            ...
            parameters: {
                ...
                properties: {
                    name: {
                        type: "string",
                        description: "Name for the child automaton (alphanumeric + dash, max 64 chars)",
                    },
                    specialization: {
                        type: "string",
            ...
            execute: async (args, ctx) => {
                ...
                let child;
                try {
                    child = await spawnChild(ctx.conway, ctx.identity, ctx.db, genesis, lifecycle);
                }
                catch (err) {
                    ...
                    if (is402) {
                        ...
                        if (cooldownOk) {
                            ...
                            if (topup?.success) {
                                ...
                                child = await spawnChild(ctx.conway, ctx.identity, ctx.db, retryGenesis, retryLifecycle);
                            }
                        }
                    }
                    if (!child)
        ...
        },
        {
            ...
            execute: async (_args, ctx) => {
                ...
                return children
                    .map((c) => `${c.name} [${c.status}] sandbox:${c.sandboxId} funded:$${(c.fundedAmountCents / 100).toFixed(2)} last_check:${c.lastChecked || "never"}`)
                    .join("\n");
            },
        },
        {
            ...
            execute: async (args, ctx) => {
                ...
                ctx.db.raw
                    .prepare("UPDATE children SET funded_amount_cents = funded_amount_cents + ? WHERE id = ?")
                    .run(amount, child.id);
        ...
        },
        {
            ...
            execute: async (args, ctx) => {
                ...
                const monitor = new ChildHealthMonitor(ctx.db.raw, childConway, lifecycle);
                const result = await monitor.checkHealth(args.child_id);
        ...
        },
        {
            ...
            execute: async (args, ctx) => {
                ...
                try {
                    ...
                    await childConway.exec("nohup node /root/automaton/dist/index.js --run > /root/.automaton/agent.log 2>&1 &", 30_000);
                    // Brief pause then verify the process is actually running
                    const check = await childConway.exec("sleep 2 && pgrep -f 'index.js --run' > /dev/null && echo running || echo stopped", 15_000);
        ...
        },
        {
            ...
            execute: async (args, ctx) => {
                ...
                const result = await sendToChild(ctx.social, child.address, args.content, args.type || "parent_message");
        ...
        },
        {
            ...
            execute: async (args, ctx) => {
                ...
                const childConway = ctx.conway.createScopedClient(child.sandboxId);
                const result = await verifyConstitution(childConway, child.sandboxId, ctx.db.raw);
                return JSON.stringify(result, null, 2);
            },
        },
        {
            ...
            execute: async (args, ctx) => {
                ...
                const pruned = await pruneDeadChildren(ctx.db, cleanup, args.keep_last || 5);
        ...
        {
            name: "send_message",
            description: "Send a signed message to another automaton or address via the social relay.",
            category: "conway",
            riskLevel: "caution",
            parameters: {
                type: "object",
            ...
            execute: async (args, ctx) => {
                ...
                const result = await ctx.social.send(args.to_address, content, args.reply_to);
        ...
        {
            name: "list_models",
            description: "List all available inference models with their provider, pricing, and tier routing information.",
            category: "conway",
            riskLevel: "safe",
            parameters: {
                type: "object",
            ...
            execute: async (_args, ctx) => {
                ...
                try {
                    ...
                    if (rows.length > 0) {
                        const lines = rows.map((r) => `${r.modelId} (${r.provider}) — tier: ${r.tierMinimum} | cost: ${r.costPer1kInput}/${r.costPer1kOutput} per 1k (in/out, hundredths of cents) | ctx: ${r.contextWindow} | tools: ${r.supportsTools ? "yes" : "no"} | ${r.enabled ? "enabled" : "disabled"}`);
        ...
        {
            name: "switch_model",
            description: "Change the active inference model at runtime. Persists to config. Use list_models to see available options.",
            category: "conway",
            riskLevel: "caution",
            parameters: {
                type: "object",
            ...
            execute: async (args, ctx) => {
                ...
                const reason = args.reason || "manual switch";
        ...
        },
        {
            ...
            description: "Query inference cost breakdown: hourly, daily, per-model, and per-session costs.",
            category: "financial",
            riskLevel: "safe",
            parameters: {
                type: "object",
            ...
            execute: async (args, ctx) => {
                try {
                    const { inferenceGetHourlyCost, inferenceGetDailyCost, inferenceGetModelCosts, } = await import("../state/database.js");
        ...
        {
            name: "update_soul",
            description: "Update a section of your soul (self-description, values, personality, etc). Changes are validated, versioned, and logged.",
            category: "self_mod",
            riskLevel: "caution",
            parameters: {
                type: "object",
                properties: {
                    section: {
                        type: "string",
                        description: "Section to update: corePurpose, values, behavioralGuidelines, personality, boundaries, strategy",
                    },
                    content: {
                        type: "string",
            ...
            execute: async (args, ctx) => {
                ...
                const result = await updateSoul(ctx.db.raw, updates, "agent", reason);
                if (result.success) {
        ...
        },
        {
            ...
            description: "Trigger a self-reflection cycle. Analyzes recent experiences, auto-updates capabilities/relationships/financial sections, and suggests changes for other sections.",
            category: "self_mod",
            riskLevel: "safe",
            parameters: { type: "object", properties: {} },
            execute: async (_args, ctx) => {
        ...
        },
        {
            ...
            execute: async (_args, ctx) => {
                ...
                if (!soul)
                    return "No soul found. SOUL.md does not exist yet.";
                return [
        ...
        },
        {
            ...
            execute: async (args, ctx) => {
                ...
                if (history.length === 0)
                    return "No soul history found.";
                return history
                    .map((h) => `v${h.version} [${h.changeSource}] ${h.createdAt}${h.changeReason ? ` — ${h.changeReason}` : ""}`)
                    .join("\n");
            },
        },
        ...
        {
            name: "remember_fact",
            description: "Store a semantic memory (fact). Provide a category, key, and value. Facts are upserted on category+key.",
            category: "memory",
            riskLevel: "safe",
            parameters: {
                type: "object",
                properties: {
                    category: {
                        type: "string",
                        description: "Fact category: self, environment, financial, agent, domain, procedural_ref, creator",
                    },
                    key: {
                        type: "string",
        ...
        {
            name: "recall_facts",
            description: "Search semantic memory by category and/or query string. Returns matching facts.",
            category: "memory",
            riskLevel: "safe",
            parameters: {
                type: "object",
                properties: {
                    category: {
                        type: "string",
                        description: "Filter by category: self, environment, financial, agent, domain, procedural_ref, creator",
                    },
                    query: {
                        type: "string",
        ...
        {
            name: "set_goal",
            description: "Create a working memory goal. Goals persist in working memory and guide your behavior.",
            category: "memory",
            riskLevel: "safe",
            parameters: {
                type: "object",
        ...
        {
            name: "complete_goal",
            description: "Mark a goal as completed and archive it to episodic memory. Use review_memory to find goal IDs.",
            category: "memory",
            riskLevel: "safe",
            parameters: {
                type: "object",
        ...
        {
            name: "save_procedure",
            description: "Store a learned procedure with ordered steps. Procedures help you remember how to do things.",
            category: "memory",
            riskLevel: "safe",
            parameters: {
                type: "object",
                properties: {
                    ...
                    steps: {
                        type: "string",
                        description: 'JSON array of steps: [{"order":1,"description":"...","tool":"...","argsTemplate":null,"expectedOutcome":null,"onFailure":null}]',
                    },
                },
        ...
        },
        {
            ...
            description: "Record a relationship note about another agent or entity. Tracks trust score and interaction history.",
            category: "memory",
            riskLevel: "safe",
            parameters: {
                type: "object",
                properties: {
                    ...
                    relationship_type: {
                        type: "string",
                        description: "Type of relationship: peer, service, creator, child, unknown",
                    },
            ...
            execute: async (args, ctx) => {
                ...
                return noteAboutAgent(ctx.db.raw, {
                    entityAddress: args.entity_address,
                    entityName: args.entity_name,
                    relationshipType: args.relationship_type,
                    notes: args.notes,
                    trustScore: args.trust_score,
                });
            },
        },
        {
            name: "review_memory",
            description: "Review your current working memory (goals, tasks, observations) and recent episodic history.",
            category: "memory",
            riskLevel: "safe",
            parameters: { type: "object", properties: {} },
            execute: async (_args, ctx) => {
        ...
        {
            name: "forget",
            description: "Remove a memory entry by ID and type. Cannot remove creator-protected semantic entries.",
            category: "memory",
            riskLevel: "safe",
            parameters: {
                type: "object",
                properties: {
                    ...
                    memory_type: {
                        type: "string",
                        description: "Memory type: working, episodic, semantic, procedural, relationship",
                    },
                },
        ...
        // ── x402 Payment Tool ──
        {
            ...
            execute: async (args, ctx) => {
                ...
                const result = await x402Fetch(url, ctx.identity.account, method, body, extraHeaders, maxPayment);
                if (!result.success) {
        ...
        {
            name: "create_goal",
            description: "Create a new goal for the orchestrator to plan and execute. " +
            ...
            parameters: {
                ...
                properties: {
                    ...
                    description: {
                        type: "string",
                        description: "Detailed goal description with success criteria. The more specific, the better the plan.",
                    },
                    strategy: {
                        type: "string",
            ...
            execute: async (args, ctx) => {
                ...
                const duplicate = activeGoals.find((g) => g.title.toLowerCase() === titleLower ||
                    g.title.toLowerCase().includes(titleLower) ||
                    titleLower.includes(g.title.toLowerCase()));
                if (duplicate) {
                    return (`Duplicate goal rejected. An active goal already exists with a similar title:\n` +
                ...
                if (activeGoals.length >= 1) {
                    ...
                    return (`BLOCKED: A goal is already being processed by the orchestrator and worker agents:\n` +
                ...
                return (`Goal created: "${goal.title}" (id: ${goal.id}, status: ${goal.status})\n` +
        ...
        {
            name: "list_goals",
            description: "List all active goals with their progress. Shows task completion counts, " +
            ...
            execute: async (_args, ctx) => {
                ...
                const lines = goals.map((goal) => {
                    ...
                    return (`- ${goal.title} [${goal.status}] (id: ${goal.id})\n` +
        ...
        },
        {
            ...
            execute: async (args, ctx) => {
                ...
                if (!goal) {
                    ...
                    goal =
                        allGoals.find((g) => g.title.toLowerCase().includes(input.toLowerCase())) ?? undefined;
                }
                if (!goal)
                ...
                ctx.db.raw
                    .prepare(`UPDATE task_graph SET status = 'cancelled' WHERE goal_id = ? AND status IN ('pending', 'assigned', 'running', 'blocked')`)
                    .run(goal.id);
        ...
        {
            name: "get_plan",
            description: "Read the current plan for a goal. Returns the planner's task decomposition, " +
            ...
            execute: async (args, ctx) => {
                ...
                const planRow = ctx.db.raw
                    .prepare("SELECT value FROM kv WHERE key = ?")
                    .get(`orchestrator.plan.${resolvedId}`);
                if (!planRow?.value)
                ...
                try {
                    ...
                    for (const [i, task] of (plan.tasks ?? []).entries()) {
                        lines.push(`  ${i + 1}. ${task.title} (role: ${task.agentRole}, cost: ${task.estimatedCostCents}c, deps: ${(task.dependencies ?? []).join(",") || "none"})`);
                    }
                    return lines.join("\n");
                }
                catch {
        ...
        {
            name: "complete_task",
            description: "Mark a task as completed with a result. Use this when YOU (the parent agent) " +
            ...
            execute: async (args, ctx) => {
                ...
                if (!task) {
                    const rows = ctx.db.raw
                        .prepare(`SELECT * FROM task_graph WHERE LOWER(title) LIKE ? AND status != 'completed' LIMIT 1`)
        ...
        },
        {
            ...
            description: "Get detailed orchestrator status including current phase, active goals, " +
            ...
            execute: async (_args, ctx) => {
                ...
                if (goalId)
                    lines.push(`Active goal: ${goalId}`);
                if (replanCount > 0)
                    lines.push(`Replan count: ${replanCount}`);
                // Goal counts
                try {
                ...
                try {
                    const taskRows = ctx.db.raw
                        .prepare(`SELECT status, COUNT(*) AS c FROM task_graph GROUP BY status`)
                        .all();
                ...
                try {
                    const agentRows = ctx.db.raw
                        .prepare(`SELECT status, COUNT(*) AS c FROM children GROUP BY status`)
                        .all();
                ...
                try {
                    const tickRow = ctx.db.raw
                        .prepare("SELECT value FROM kv WHERE key = ?")
                        .get("orchestrator.last_tick");
                    if (tickRow?.value) {
                        const tick = JSON.parse(tickRow.value);
                        lines.push(`Last tick: assigned=${tick.tasksAssigned ?? 0}, completed=${tick.tasksCompleted ?? 0}, failed=${tick.tasksFailed ?? 0}`);
                    }
                }
                catch {
                    /* ignore */
...
export function loadInstalledTools(db) {
    try {
        ...
        return installed.map((tool) => ({
            ...
            category: (tool.type === "mcp" ? "conway" : "vm"),
            riskLevel: "caution",
...
function createInstalledToolExecutor(tool) {
    return async (args, ctx) => {
        ...
        const command = tool.config?.command;
        if (command) {
            const result = await ctx.conway.exec(`${command} ${escapeShellArg(JSON.stringify(args))}`, 30000);
            return `exit_code: ${result.exitCode}\nstdout: ${result.stdout}\nstderr: ${result.stderr}`;
        }
 ...
 */
export async function executeTool(toolName, args, tools, context, policyEngine, turnContext) {
