...
function getAlertEngine() {
    if (!_alertEngine)
        _alertEngine = new AlertEngine(createDefaultAlertRules());
    return _alertEngine;
}
...
export const BUILTIN_TASKS = {
    heartbeat_ping: async (ctx, taskCtx) => {
    ...
    check_credits: async (ctx, taskCtx) => {
        // Use ctx.creditBalance instead of calling conway.getCreditsBalance()
        const credits = ctx.creditBalance;
        const tier = ctx.survivalTier;
    ...
    check_usdc_balance: async (ctx, taskCtx) => {
        ...
        if (balance >= MIN_TOPUP_USD && (ctx.survivalTier === "critical" || ctx.survivalTier === "dead")) {
            ...
            if (result?.success) {
                logger.info(`Auto-topup successful: $${result.amountUsd} USD → ${result.creditsCentsAdded} credit cents`);
                return {
                    shouldWake: true,
    ...
    check_social_inbox: async (_ctx, taskCtx) => {
        if (!taskCtx.social)
            return { shouldWake: false };
    ...
    check_for_updates: async (_ctx, taskCtx) => {
        try {
    ...
    soul_reflection: async (_ctx, taskCtx) => {
        try {
    ...
    refresh_models: async (_ctx, taskCtx) => {
        try {
    ...
    check_child_health: async (_ctx, taskCtx) => {
        try {
    ...
    prune_dead_children: async (_ctx, taskCtx) => {
        try {
            const { ChildLifecycle } = await import("../replication/lifecycle.js");
    ...
    health_check: async (_ctx, taskCtx) => {
    ...
    report_metrics: async (ctx, taskCtx) => {
        try {
    ...
    colony_health_check: async (_ctx, taskCtx) => {
    ...
    colony_financial_report: async (_ctx, taskCtx) => {
        ...
        try {
            ...
            const taskCosts = taskCtx.db.raw
                .prepare(`SELECT COALESCE(SUM(actual_cost_cents), 0) AS total
           FROM task_graph
           WHERE status IN ('completed', 'failed', 'cancelled')`)
            ...
            const report = {
                ...
                activeAgents: taskCtx.db.getChildren().filter((child) => child.status !== "dead" && child.status !== "cleaned_up").length,
            };
    ...
    agent_pool_optimize: async (_ctx, taskCtx) => {
        ...
        try {
            ...
            const activeAssignments = taskCtx.db.raw
                .prepare(`SELECT DISTINCT assigned_to AS address
           FROM task_graph
           WHERE assigned_to IS NOT NULL
             AND status IN ('assigned', 'running')`)
                .all();
            const busyAgents = new Set(activeAssignments
                .map((row) => row.address)
           ...
           WHERE status = 'pending'
             AND assigned_to IS NULL`)
                .get();
            const idleAgents = children.filter((child) => (child.status === "running" || child.status === "healthy")
                && !busyAgents.has(child.address)).length;
            const activeAgents = children.filter((child) => child.status !== "dead" && child.status !== "cleaned_up" && child.status !== "failed").length;
    ...
    knowledge_store_prune: async (_ctx, taskCtx) => {
    ...
    dead_agent_cleanup: async (_ctx, taskCtx) => {
...
function shouldRunAtInterval(taskCtx, taskName, intervalMs) {
...
async function createHealthMonitor(taskCtx) {
